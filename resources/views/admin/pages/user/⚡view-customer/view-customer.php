...
new #[Layout('layouts.app-admin')] class extends Component
{
    ...
    {
        User::role(['customer'])->whereIn('id', $this->selectedUsers)->delete();
    ...
    { 
        ...
        { 
            ...
            ->filter(fn($user) => $user->hasRole(['customer']))
            ->pluck('id')
            ->map(fn($id) => (int) $id)
            ->toArray(); 
        } else { 
    ...
    {
        $allIds = User::role(['customer'])->pluck('id')->map(fn($id) => (int) $id)->toArray();
