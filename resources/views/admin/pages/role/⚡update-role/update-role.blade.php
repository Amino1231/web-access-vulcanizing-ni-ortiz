<div>
    <div class="max-w-[85rem] px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mx-auto rounded-xl">
        <div class="mt-12 max-w-full mx-auto">
            <!-- Card -->
            <div class="flex flex-col border border-neutral rounded-xl p-4 sm:p-6 lg:p-8 bg-gradient-to-br from-orange-600/20 via-transparent to-transparent shadow-2xl">
                    ...
                    <div class="grid gap-4 lg:gap-6">
                        ...
                        <div>
                            ...
                            <input wire:model.defer='name' type="text" name="hs-name" id="hs-name"
                                class="py-2.5 sm:py-3 px-4 block w-full border border-black rounded-lg sm:text-sm text-white placeholder:text-white focus:outline-none">
                        </div>
                        ...
                        <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-2">
                            @forelse ($this->permissions as $pKey => $permission)
                                <label wire:key='{{ $pKey }}' for="hs-checkbox-in-form-{{ $pKey }}"
                                    class="flex items-center p-3 w-full bg-orange-600 shadow-2xl border-transparent rounded-lg text-sm hover:bg-orange-400 transition">
                                    <input type="checkbox" id="hs-checkbox-in-form-{{ $pKey }}"
                                        wire:model='selectedPermissions' value="{{ $permission->name }}"
                                        class="shrink-0 size-4 rounded-sm">
