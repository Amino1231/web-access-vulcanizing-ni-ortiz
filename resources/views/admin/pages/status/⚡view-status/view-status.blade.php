                                ...
                                @if(!empty($selectedStatus))
                                    <div class="py-2">
                                        <button 
                                            x-data 
                                            @click="
                                                if (confirm('Are you sure you want to delete ' + {{ count($selectedStatus) }} + ' status(s)?')) {
                                                    $wire.deleteSelected()
                                                }
                                            "
                                            id="btn-cancel"
                                            class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">
                                            Delete Selected ({{ count($selectedStatus) }})
                                        </button>
                                    </div>`
                                @endif
                            </div>
                                ...
                                <tr>
                                    <th class="ps-2 sm:ps-6 py-3">
                                        <input type="checkbox"
                                            ...
                                            @checked($selectAll)
                                            x-data
                                            x-init="$watch('$wire.selectedStatus', value => {
                                ...
                                @forelse ($this->statuses as $status)
                                    <tr>
                                        <td class="w-4 ps-2 sm:ps-6 py-3 text-center align-middle">
                                            <input type="checkbox"
                                                wire:click="toggleRowSelection({{ $status->id }})"
                                                x-data
                                                x-bind:checked="@js($selectedStatus).includes({{ $status->id }})"
                                        ...
                                        <td class="px-2 sm:px-6 py-1.5 text-start">
                                            <a href="{{ route('admin.status.update', $status->id) }}"
