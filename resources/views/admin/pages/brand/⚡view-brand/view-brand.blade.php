                                ...
                                @if(!empty($selectedBrands))
                                    <div class="py-2">
                                        <button 
                                            x-data 
                                            @click="
                                                if (confirm('Are you sure you want to delete ' + {{ count($selectedBrands) }} + ' brand(s)?')) {
                                                    $wire.deleteSelected()
                                                }
                                            "
                                            id="btn-cancel"
                                            class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">
                                            Delete Selected ({{ count($selectedBrands) }})
                                        </button>
                                    </div>`
                                @endif
                            </div>
                                ...
                                <tr>
                                    <th class="ps-2 sm:ps-6 py-3">
                                        <input type="checkbox"
                                            ...
                                            @checked($selectAll)
                                            x-data
                                            x-init="$watch('$wire.selectedBrands', value => {
                                                const total = {{ $this->totalBrandsCount }}; 
                                ...
                                @forelse ($this->brands as $brand)
                                    <tr>
                                        <td class="w-4 ps-2 sm:ps-6 py-3 text-center align-middle">
                                            <input type="checkbox"
                                                wire:click="toggleRowSelection({{ $brand->id }})"
                                                x-data
                                                x-bind:checked="@js($selectedBrands).includes({{ $brand->id }})"
